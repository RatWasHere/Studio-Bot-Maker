module.exports = {
  data: {
    name: "Find Channel",
    storeAs: "",
    button: "Command Guild",
    ExtraData: "",
    findVia: "Channel Name*",
    via: "",
  },
  UI: {
  ...
  async run(values, message, uID, fs, client, runner, bridge) {
    ...
    if (values.findVia == "Channel Name*") {
      filter = (m) =>
        m.name.includes(varTools.transf(values.via, bridge.variables));
    } else {
    ...
    bridge.variables[varTools.transf(values.storeAs, bridge.variables)] =
      await guild.channels.find(filter);
  },
};
