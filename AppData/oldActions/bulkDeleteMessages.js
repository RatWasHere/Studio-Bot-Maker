module.exports = {
    ...
    UI: {"compatibleWith": ["Text", "Slash"], "text":"Bulk Delete", "sepbar":"",
    "btext":"Get Messages Channel Via",
    "menuBar": {"choices": ["Command Channel", "Variable*", "ID*"], "storeAs":"channelFrom", "extraField":"channelVariable"},
    "sepbar0":"",
    ...
    "sepbar2":"", "btext2":"Get User To Delete Messages From Via",
    "menuBar2": {"choices": ["Command Author", "User*", "ID*", "Anybody"], "storeAs":"user", "extraField":"fromWho"},
    "invisible":"",
    "variableSettings": {
        "channelVariable": {
            "Variable*": "direct"
    ...
    run(values, message, uID, fs, client, runner, bridge) {
        ...
        if (values.channelFrom == 'Variable*') {
            const channelID = bridge.variables[varTools.transf(values.channelVariable, bridge.variables)].id;
            channel = client.getChannel(channelID);
        }
        if (values.channelFrom == 'ID*') {
            channel = client.getChannel(varTools.transf(values.channelVariable, bridge.variables))
        }
        ...
        if (values.user == 'User*') {
            mustBeFrom = bridge.variables[varTools.transf(values.fromWho, bridge.variables)].id
        }
        if (values.user == 'ID*') {
        ...
        let checkFor = m => m.content.toLowerCase().includes(mustInclude.toLowerCase())
