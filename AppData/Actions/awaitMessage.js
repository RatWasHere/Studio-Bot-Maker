module.exports = {
    ...
    "menuBar0":{"choices":["Anybody", "Command Author", "User*", "User ID*"], storeAs: "targetUser", extraField:"fromWho"},  
    ...
    async run(values, inter, uID, fs, client, actionRunner, bridge) { 
        ...
        const handlemessage = (message) => {
            ...
            switch (values.channelFrom) {
                ...
                break
                case 'ID*':
                    matchesTarget = varTools.transf(values.channel, bridge.variables) == message.channel.id
                break
                case 'Variable*':
                    matchesTarget = bridge.variables[varTools.transf(values.channel, bridge.variables)].id == message.channel.id;
                break
            }
            ...
            switch (values.targetUser) {
                ...
                break
                case 'User*':
                    matchesTarget = message.author.id == bridge.variables[varTools.transf(values.fromWho, bridge.variables)].id
                break
                case 'User ID*':
                    matchesTarget = message.author.id == varTools.transf(values.fromWho, bridge.variables)
                break
            }

            if (matchesTarget && matchesChannel) {
                actionRunner(values.actions, message, client, {...bridge.variables, [varTools.transf(values.storeMessageAs, bridge.variables)]: message, [varTools.transf(values.storeMessageAuthorAs, bridge.variables)]: message.author}, true);
            }
        }

        client.on('messageCreate', handlemessage(message, message.author, reaction))
