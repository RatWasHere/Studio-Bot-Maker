module.exports = {
    ...
    "menuBar": {"choices": ["Command Channel", "Command Author", "Channel Variable*", "Channel ID*", "User Variable*", "User ID*"], storeAs: "sendTo", extraField: "to"},
    "sepbar3":"",
    ...
    async run(values, message, uID, fs, client, actionRunner, bridge) {
        ...
        if (values.sendTo == 'Channel Variable*') {
            channel = client.getChannel(bridge.variables[varTools.transf(values.to, bridge.variables)].id);
        }
        ...
        if (values.sendTo == 'User Variable*') {
            const DMchannel = await client.users.get(bridge.variables[varTools.transf(values.to, bridge.variables)].id).createDM()
            channel = DMchannel
        }
