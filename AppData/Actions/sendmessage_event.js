module.exports = {
    data: {"messageContent": "", "storeAs":"", "sendTo":"Channel*", "to": "", "actionRows":[], "name": "Send Message", "ExtraData": "", "sendTo":""},
    ...
    "menuBar": {"choices": ["Command Channel", "Command Author", "Channel*", "Channel ID*", "User*", "User ID*"], storeAs: "sendTo", extraField: "to"},
    "sepbar3":"",
    ...
    async run(values, message, uID, fs, client, actionRunner) {
        ...
        let channel;
        if (values.sendTo == 'Channel*') {
            channel = client.getChannel(tempVars[uID][varTools.transf(values.to, uID, tempVars)].id);
        }
        if (values.sendTo == 'Channel ID*') {
            channel = client.getChannel(varTools.transf(values.to, uID, tempVars));
        }
        if (values.sendTo == 'User*') {
            const DMchannel = await client.users.get(tempVars[uID][varTools.transf(values.to, uID, tempVars)].id).createDM()
            channel = DMchannel
        }
        if (values.sendTo == 'User ID*') {
            const DMchannel = await client.users.get(varTools.transf(values.to, uID, tempVars)).createDM()
            channel = DMchannel
        }

        channel.createMessage({content: varTools.transf(values.messageContent, uID, tempVrz), embeds: embeds , components: endComponents}).then(msg => {
            if (values.storeAs != "") {
